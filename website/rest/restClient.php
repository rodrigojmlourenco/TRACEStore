...
function signinRest($username, $password) {

	$api = new RestClient(array(
		'base_url' => "http://146.193.41.50:8080/trace", 
	));
	$api->set_option('user_agent', "application/json");

	//	$result = $api->post("auth/login",array('username' => $_SESSION["username"],'password' => $_SESSION["password"]));
	$result = $api->post("auth/login",array('username' => $username,'password' => $password));
	$loginSuccessful = json_decode($result->response)->success;

	return $loginSuccessful;
}

function registerRest($name, $address, $phone, $email, $username, $password, $confirm) {
	...
	$result = $api->post("tracker/register", array('name' => $name,'username' => $username, 'email' => $email, 'password' => $password, 'confirm' => phone, 'phone' => $phone, 'address' => $address));
	$loginSuccessful = json_decode($result->response)->success;
